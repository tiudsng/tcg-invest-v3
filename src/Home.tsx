        ...
        <div className="flex w-full gap-2">
          <div className="relative flex-grow md:w-96">
            ...
            <input
              ...
              className="block w-full pl-11 pr-4 py-3.5 bg-gray-100/80 dark:bg-white/5 border-0 rounded-2xl text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-white/10 transition-all shadow-sm text-sm"
            />
          </div>
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={`px-4 py-3.5 rounded-2xl border transition-all flex items-center gap-2 font-bold ${
      ...
      {activeTab === 'sell' ? (
        loading ? (
          ...
          <div className="text-center py-24 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border border-gray-100 dark:border-gray-700">
            ...
            <button onClick={clearFilters} className="text-blue-600 font-bold">清除所有篩選</button>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
            {filteredListings.map((listing, index) => (
              ...
              >
                <div onClick={() => navigate(`/listing/${listing.id}`)} className="group block h-full cursor-pointer">
                  <div className="bg-white dark:bg-[#1c1c1e] rounded-3xl overflow-hidden shadow-sm border border-gray-100 dark:border-white/5 hover:shadow-xl transition-all duration-300 h-full flex flex-col">
                    <div className="aspect-[3/4] w-full overflow-hidden bg-gray-100 dark:bg-black relative">
                      <img src={listing.imageUrl} alt={listing.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer" loading="lazy" decoding="async" />
                      {listing.imageUrls && listing.imageUrls.length > 1 && (
                        <div className="absolute bottom-1.5 right-1.5 px-2 py-0.5 bg-black/50 backdrop-blur-md rounded-lg text-[10px] font-bold text-white flex items-center gap-1 z-10">
                          <ImageIcon className="w-3 h-3" />
                          <span>{listing.imageUrls.length}</span>
                        </div>
                      )}
                      <ConditionBadge condition={listing.condition} cardType={listing.cardType} title={listing.title} className="absolute top-1.5 right-1.5 sm:top-3 sm:right-3 !h-6 sm:!h-8" />
                    ...
                    <div className="p-2.5 sm:p-5 flex flex-col flex-grow">
                      <h3 className="text-[13px] sm:text-lg font-bold text-gray-900 dark:text-white line-clamp-1 mb-0.5 sm:mb-1">{listing.title}</h3>
                      ...
                      <div className="mt-auto pt-2 sm:pt-4 border-t border-gray-100 dark:border-white/5">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5 truncate">
                            {listing.sellerPhoto ? (
                              <img src={listing.sellerPhoto} alt={listing.sellerName} className="w-4 h-4 sm:w-6 sm:h-6 rounded-full" referrerPolicy="no-referrer" />
                            ) : (
                              <div className="w-4 h-4 sm:w-6 sm:h-6 bg-gray-200 dark:bg-white/10 rounded-full" />
                            )}
            ...
            {isFetchingMore && <div className="col-span-full flex justify-center py-8"><Loader2 className="w-8 h-8 animate-spin text-blue-600" /></div>}
          </div>
        )
      ) : (
        loadingWants ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {[...Array(6)].map((_, i) => <div key={i} className="bg-white dark:bg-[#1c1c1e] rounded-3xl p-6 border border-gray-100 dark:border-white/5 animate-pulse h-32" />)}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
            {filteredWantListings.map((listing, index) => (
              <motion.div
                ...
                className="bg-white dark:bg-[#1c1c1e] rounded-3xl p-4 sm:p-5 shadow-sm border border-gray-100 dark:border-white/5 flex flex-col h-full"
              >
                <div className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-4">
                  {listing.imageUrl ? (
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden shrink-0 border border-gray-200 dark:border-white/10">
                      <img src={listing.imageUrl} alt={listing.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    </div>
                  ) : (
                    <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gray-50 dark:bg-black/20 flex items-center justify-center shrink-0">
                      <Search className="w-6 h-6 text-gray-400" />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap gap-1 mb-1">
                      <ConditionBadge condition={listing.condition || 'Mint'} cardType={listing.cardType} title={listing.title} className="!px-2 !py-0.5 !text-[8px] !h-auto" />
                    </div>
                    <h3 className="text-sm sm:text-lg font-bold text-gray-900 dark:text-white line-clamp-2 mb-1">{listing.title}</h3>
                ...
                <div className="mt-auto pt-3 border-t border-gray-100 dark:border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {listing.userPhoto ? (
                      <img src={listing.userPhoto} alt={listing.userName} className="w-5 h-5 rounded-full" referrerPolicy="no-referrer" />
                    ) : (
                      <div className="w-5 h-5 bg-gray-200 dark:bg-white/10 rounded-full" />
                    )}
                    <span className="text-xs text-gray-600 dark:text-gray-300 font-medium truncate max-w-[80px]">{listing.userName}</span>
                  </div>
            ...
            {isFetchingMoreWants && <div className="col-span-full flex justify-center py-8"><Loader2 className="w-8 h-8 animate-spin text-blue-600" /></div>}
          </div>
        )
      )}
    </div>
